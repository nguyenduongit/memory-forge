  ...
  return <div className="bright-app"><div className="ambient ambient--one" /><div className="ambient ambient--two" />
    <main className="mobile-canvas">
      {screen === "modules" && <section className="scene scene--launchpad" aria-label="Chọn module ghi nhớ">
        <div className="launchpad-list">{modules.map((module) => <button key={module.id} disabled={!module.enabled} onClick={() => module.enabled && setScreen("mode")} className={`launchpad-card launchpad-card--${module.tone} ${!module.enabled ? "launchpad-card--locked" : ""}`} aria-label={module.enabled ? module.title : `${module.title}, chưa khả dụng`}>
          <span className="launchpad-card__icon">{module.enabled ? module.icon : <Lock size={23} strokeWidth={2.2} />}</span>
